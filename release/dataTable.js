      ...
      compile: function compile() {
        return {
          pre: function pre($scope, $elm, $attrs, ctrl) {
            ...
            $scope.$watch('cell.row', function (newVal) {
              if (cellScope) {
  ...
  var BodyController = function () {
    ...
    _createClass(BodyController, [{
          ...
          if (this.options.scrollbarV) {
            ...
            if (this.options.paging.externalPaging) {
              ...
              if (Object.keys(this.expanded).length > 0) {
                var keys = [], numOfChildren = 0;
                for (var e in this.expanded) {
                  if (this.expanded[e]) keys.push(e);
                }
                for (var k in keys) {
                  if (this.rowsByGroup[keys[k]] && this.rowsByGroup[keys[k]].length > 1) {
                    numOfChildren = numOfChildren + this.rowsByGroup[keys[k]].length;
                  }
                }
                if (numOfChildren > 0) idxs.last = idxs.last + numOfChildren;
              }
      ...
      value: function rowStyles(row) {
        ...
        if (this.expanded) {
          if (this.selected && (row.invoiceId === this.selected || row.parentInvoiceId === this.selected)) {
            if (this.expanded[this.selected]) {
              styles.background = "#E3E3E3";
            } else {
  ...
  var HeaderCellController = function () {
    ...
    _createClass(HeaderCellController, [{
      ...
      value: function sortClass() {
        return {
          'sort-btn': true,
          'sort-asc icon-down-sort': this.column.sort === 'asc',
